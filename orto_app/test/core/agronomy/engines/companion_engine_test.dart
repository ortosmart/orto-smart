...
void main() {
  group('CompanionEngine', () {
    test('trova una regola esistente', () {
      final rule = CompanionEngine.findRule(CropIds.pomodoro, CropIds.basilico);
    ...
    test('riconosce la regola anche invertendo le colture', () {
      final rule = CompanionEngine.findRule(CropIds.basilico, CropIds.pomodoro);
    ...
    test('restituisce null se non esiste alcuna regola', () {
      final rule = CompanionEngine.findRule(CropIds.lattuga, '999');

      expect(rule, isNull);
    });

    test(
      'analyze restituisce un risultato eccellente per pomodoro e basilico',
      () {
        ...
        expect(result.compatibility, CompanionCompatibility.excellent);
        expect(result.message, isNotEmpty);
      },
    );

    test('analyze restituisce neutrale quando non esiste una regola', () {
      final result = CompanionEngine.analyze(CropIds.lattuga, '999');

      expect(result.compatible, isTrue);
      expect(result.compatibility, CompanionCompatibility.neutral);
      expect(
        result.message,
