...
void main() {
  ...
  group('RotationEngine.evaluate', () {
    test(
      ...
      () {
        final history = [
          createPlanting(id: 'lattuga-2025', crop: lattuga, year: 2025),
        ];
        ...
        expect(
          result.reasons.any(
            (reason) => reason.contains('Nessuna coltura della famiglia'),
          ),
          isTrue,
        );
      },
    ...
    test('sconsiglia la stessa famiglia nella stagione corrente', () {
      final history = [
        createPlanting(id: 'pomodoro-2026', crop: pomodoro, year: 2026),
      ];
      ...
      expect(
        result.reasons.any((reason) => reason.contains('stagione corrente')),
        isTrue,
      );
    });

    test('considera rispettata una rotazione completa', () {
      final history = [
        createPlanting(id: 'pomodoro-2023', crop: pomodoro, year: 2023),
      ];
      ...
      expect(
        result.reasons.any(
          (reason) => reason.contains('rotazione della famiglia botanica'),
        ),
        isTrue,
      );
    });

    test('considera accettabile una rotazione parzialmente rispettata', () {
      final history = [
        createPlanting(id: 'pomodoro-2024', crop: pomodoro, year: 2024),
      ];
      ...
      expect(
        result.reasons.any(
          (reason) => reason.contains('attendere ancora 1 stagione'),
        ),
        isTrue,
      );
    });

    test('sconsiglia una rotazione ancora troppo breve', () {
      final history = [
        createPlanting(id: 'pomodoro-2025', crop: pomodoro, year: 2025),
      ];
      ...
      expect(
        result.reasons.any(
          (reason) => reason.contains('attendere ancora 2 stagioni'),
        ),
        isTrue,
      );
    });
    ...
    test(
      ...
      () {
        ...
        expect(result.reasons.single, contains('famiglia botanica'));
      },
    );
    ...
    test(
      ...
      () {
        final history = [
          createPlanting(id: 'pomodoro-2025', crop: pomodoro, year: 2025),
          createPlanting(id: 'peperone-2024', crop: peperone, year: 2024),
        ];
        ...
        expect(
          result.reasons.any((reason) => reason.contains('compare 2 volte')),
          isTrue,
        );
      },
    );

    test('ignora le piantagioni successive alla data di valutazione', () {
      final history = [
        createPlanting(id: 'pomodoro-2027', crop: pomodoro, year: 2027),
      ];
  ...
  group('RotationEngine.evaluateCandidates', () {
    test('ordina le colture candidate dal punteggio più alto al più basso', () {
      final history = [
        createPlanting(id: 'pomodoro-2026', crop: pomodoro, year: 2026),
        createPlanting(id: 'lattuga-2025', crop: lattuga, year: 2025),
      ];
      ...
      final evaluations = RotationEngine.evaluateCandidates(
        candidateCrops: const [pomodoro, lattuga, basilico],
        history: history,
        cropsById: cropsById,
        referenceDate: DateTime(2026, 4, 1),
      );
      ...
      for (var index = 0; index < evaluations.length - 1; index++) {
        expect(
          evaluations[index].result.score,
          greaterThanOrEqualTo(evaluations[index + 1].result.score),
        );
      }
    });

    test('ordina alfabeticamente le colture con lo stesso punteggio', () {
      final evaluations = RotationEngine.evaluateCandidates(
        candidateCrops: const [lattuga, basilico],
        history: const [],
        cropsById: cropsById,
        referenceDate: DateTime(2026, 4, 1),
      );
