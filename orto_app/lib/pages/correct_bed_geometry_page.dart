...
class CorrectBedGeometryPage extends StatefulWidget {
  final Bed bed;
  final BedRepository repository;
  final ProfileWriteAuthorityController authority;

  const CorrectBedGeometryPage({
    super.key,
    required this.bed,
  ...
  @override
  State<CorrectBedGeometryPage> createState() => _CorrectBedGeometryPageState();
}

class _CorrectBedGeometryPageState extends State<CorrectBedGeometryPage> {
  ...
  String? _requiredReason(String? text) {
    if ((text ?? '').trim().isEmpty) {
      return 'Inserisci il motivo della correzione.';
    }
  ...
  Future<void> _save() async {
    ...
    try {
      final result = await widget.repository.correctBedGeometry(
        bedId: widget.bed.id,
        geometryId: widget.bed.geometry.id,
      ...
      if (result is BedGeometryCorrected ||
          result is CorrectBedGeometryUnchanged) {
        Navigator.of(context).pop(true);
        return;
      }

      final message = switch (result) {
        CorrectBedGeometryVersionConflict() =>
          'L’aiuola è stata modificata da un’altra sessione. '
              'Torna indietro e aggiorna i dati prima di riprovare.',
        CorrectBedGeometryForbidden() =>
          'Non sei autorizzato a correggere la geometria di questa aiuola.',
        CorrectBedGeometryWriteForbidden() =>
          'Il server non ha autorizzato la scrittura.',
        CorrectBedGeometryNotFound() =>
          'L’aiuola o la geometria non è più disponibile.',
        CorrectBedGeometryInvalidInput() =>
          'Il server ha rifiutato la correzione. '
              'Controlla i valori e la data inseriti.',
        BedGeometryCorrected() || CorrectBedGeometryUnchanged() =>
          throw StateError('Unexpected geometry correction result'),
      };
    ...
    return AnimatedBuilder(
      ...
      builder: (context, child) {
        ...
        return PopScope(
          canPop: !_saving,
          child: Scaffold(
            appBar: AppBar(title: const Text('Correggi geometria')),
            body: SingleChildScrollView(
              ...
              child: Form(
                ...
                child: Column(
                  ...
                  children: [
                    const Text(
                      'Questa funzione rettifica una geometria già registrata. '
                      'Usala solo per correggere dati storici errati.',
                    ),
                    ...
                    TextFormField(
                      ...
                      decoration: const InputDecoration(
                        labelText: 'Larghezza corretta (cm)',
                      ),
                    ...
                    TextFormField(
                      ...
                      decoration: const InputDecoration(
                        labelText: 'Lunghezza corretta (cm)',
                      ),
                      keyboardType: TextInputType.number,
                      validator: _positiveInteger,
                    ),
                    ...
                    FilledButton.icon(
                      ...
                      label: Text(
                        _saving ? 'Salvataggio…' : 'Conferma correzione',
                      ),
                    ),
                  ],
                ),
